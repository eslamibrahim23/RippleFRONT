...
const SideBar = () => {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      const response = await axios.get(
        `https://rippleback.onrender.com/user/profile/${userId}`
      );
  ...
  const saveProfileData = async () => {
    try {
      ...
      await axios.patch(
        `https://rippleback.onrender.com/user/editprofile/${userId}`,
        formData,
        {
          headers: {
  ...
  const confirmDeleteAccount = () => {
    axios
      .delete(`https://rippleback.onrender.com/user/deleteProfile/${userId}`)
      .then((response) => {
  ...
  return (
    <>
      <aside className="inset-y fixed  left-0 z-20 flex h-full flex-col border-r gap-2">
        <div className="border-b p-2">
          <Link to="/" className="rounded-lg">
            <Button variant="outline" size="icon" aria-label="Welcome Screen">
              <AudioWaveform className="size-6 fill-foreground" />
            </Button>
          </Link>
        </div>
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    ...
                    <form onSubmit={handleSubmit}>
                      <div className="grid gap-4 py-4">
                        ...
                        >
                          <AvatarImage
                            src={profileData.Image} // Display the selected image
