...
const ChatBox = ({ receiver, receiverId, senderId, chat }) => {
  ...
  useEffect(() => {
    const socketServerUrl = `https://rippleback.onrender.com`;
    ...
    const fetchMessages = async () => {
      try {
        const response = await axios.get(
          `https://rippleback.onrender.com/message/messagesChatId/${chat?._id}`
        );
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(
          `https://rippleback.onrender.com/user/profile/${receiverId}`
        );
  ...
  const onSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `https://rippleback.onrender.com/message/createMessage/${chat._id}`,
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(
          `https://rippleback.onrender.com/user/profile/${senderId}`
        );
  ...
  return (
    <>
      <div className="relative flex h-full  flex-col rounded-xl bg-muted/50 p-4 lg:col-span-3">
        <Badge variant="outline" className="absolute left-5 top-1 z-10 mt-1">
          Chat with {receiverData ? receiverData.userName : "loading"}
        </Badge>
        <div className="flex-1" />
        <ScrollArea className="h-[470px] w-full rounded-md border mb-3  flex">
          <div>
            {messages
              ? messages.map((m, i) => {
                  return (
